...
exports.run = async (client, message, args) => {
  ...
  if (sure !== null && cd - (Date.now() - sure) > 0) {
    ...
    message.channel
      .send(
        `Bir daha maden kazmak için biraz beklemen gerekli **${timeObj.seconds} saniye** sonra tekrar dene!`
      )
      .then((msg) => msg.delete({ timeout: `${cd}` }));
  } else {
    if (!hesapd)
      return message.channel.send(
        "İlk önce hesap oluşturmalısın\nHesap oluşturmak için `!hesap-oluştur <isim>`"
      );

    if (!args[0])
      return message.reply(
        `Bir maden belirtiniz. Örnek: !maden zümrüt/elmas/altın/demir/taş`
      );
    if (args[0] === "zümrüt") {
      if (!elmaskazma)
        return message.reply(`Hiç elmas kazmanız bulunmamaktadır! !market`);
...
`);
    ...
    if (args[0] === "elmas") {
      if (!demirkazma)
        return message.reply(`Hiç demir kazmanız bulunmamaktadır! !market`);
...
`);
    ...
    if (args[0] === "demir") {
      if (!taşkazma)
        return message.reply(`Hiç taş kazmanız bulunmamaktadır! !market`);
