...
**!satın-al silah-telefon-olta-bitcoin-kalkan** 
**!satın-al elmas-kazma/demir-kazma/taş-kazma** 
**!satın-al elmas-balta/demir-balta/taş-balta**
**!satın-al elmas-kılıç/demir-kılıç/taş-kılıç**`
        )
  ...
  if (args[0] === "bitcoin") {
    let fiyatcık5 = 70000; // istediğiniz fiyat
  ...
  if (args[0] === "kalkan") {
    if (bakiye < 3500) {
      if ((await db.has(`${message.author.id}.kalkan`)) === true)
        return message.reply(`**Zaten 1 adet kalkanın bulunuyor!** `);
      message.reply(
    ...
    if (bakiye > 3500) {
      if ((await db.has(`${message.author.id}.kalkan`)) === true)
        return message.reply(`**Zaten 1 adet kalkanın bulunuyor!** `);
      message.reply(
        `Marketten **1** adet \`1 Günlük Kalkan\` aldın. Şuanki paran: ${
          bakiye - 3500
        } `
      );
      ...
      client.channels.cache
        .get(soygunlog)
        .send(
          ` **Adlı kullanıcı marketten \`1 Günlük Kalkan\` satın aldı. Geriye **${
            bakiye - 3500
          }**TL'si kaldı.`
        );
    }
