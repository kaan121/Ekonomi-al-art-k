    ...
    if (!hesapismi) {
      ...
      if (client.ekoayarlar.rastgelepara == true) {
        ...
        const randomizer = getRandomInt(
          client.ekoayarlar.minpara,
          client.ekoayarlar.maxpara
        );
        db.add(`bakiye_${message.author.id}`, randomizer);
        message.channel.send(
          `Başlangıç parası rastgele olarak **${randomizer} ${client.ekoayarlar.parabirimi}** hesabınıza yatırıldı!`
        );
      } else {
        if (client.ekoayarlar.rastgelepara == false) {
          ...
          db.add(`balta_${message.author.id}`, 1);
          db.add(
            `bakiye_${message.author.id}`,
            client.ekoayarlar.başlangıçparası
          );
          message.channel.send(
            `Başlangıç parası olarak **${client.ekoayarlar.başlangıçparası} ${client.ekoayarlar.parabirimi}** hesabınıza yatırıldı!`
          );
        }
      }
    }
