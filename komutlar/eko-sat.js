...
`
        ...
        .setFooter("Crazy Of Economy", message.author.avatarURL())
        .setTimestamp()
    );
  ...
  if (args[0] === "bitcoin") {
    let fiyatcık5 = `${client.ekoayarlar.btcdeger}`; // istediğiniz fiyat

    db.delete(`btc_${kllanç.id}`, "Kapalıdır");
    db.add(`bakiye_${kllanç.id}`, +fiyatcık5);

    return message.reply(
      `Ürünü başarıyla sattınız. Kazandığınız para: ${fiyatcık5}`
    );
  }
};
exports.conf = {
