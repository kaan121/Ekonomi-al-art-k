...
exports.run = async (client, message, args) => {
  ...
  if (sure !== null && cd - (Date.now() - sure) > 0) {
    ...
    message.channel
      .send(
        `Bir daha savaşmak için biraz beklemen gerekli **${timeObj.seconds} saniye** sonra tekrar dene!`
      )
      .then((msg) => msg.delete({ timeout: `${cd}` }));
  } else {
    if (!hesapd)
      return message.channel.send(
        "İlk önce hesap oluşturmalısın\nHesap oluşturmak için `s!hesap-oluştur <isim>`"
      );
...
`);
    ...
    if (args[0] === "örümcek") {
      if (!elmaskılıç)
        return message.reply(`Hiç elmas kılıcınız bulunmamaktadır! s!market`);
...
`);
    ...
    if (args[0] === "creeper") {
      if (!demirkılıç)
        return message.reply(`Hiç taş baltanız bulunmamaktadır! s!market`);
...
`);
    ...
    if (args[0] === "enderman") {
      if (!demirkılıç)
        return message.reply(`Hiç demir kılıcınız bulunmamaktadır! s!market`);
